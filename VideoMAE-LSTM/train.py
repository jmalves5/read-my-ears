...
if __name__ == "__main__":
    ...
    sweep_id = wandb.sweep(sweep_config, project="videoMAE+LSTM_train_variations_new")
    #sweep_id = "videoMAE+LSTM_train_variations/x36m2mar"
        ...
        with wandb.init(config=config, resume=True):
            ...
            for epoch in tqdm.tqdm(range(config.epochs)):
                ...
                if val_loss < best_val_loss:
                    ...
                    torch.save(
                        model.state_dict(), 
                       f"{script_path}/checkpoints/lstm_best_epoch_accuracy_lr_{config.lr}_fps_{config.fps}_sample_{config.sample}_hidden_dim_{config.hidden_dim}_num_layers_{config.num_layers}_dropout_{config.dropout}.pth"
                    )
                    best_epoch = epoch
                else:
                    no_improve_count += 1
                ...
                if no_improve_count >= patience:
                    ...
                    # log model to wandb
                    wandb.log_model(path=f"{script_path}/checkpoints/lstm_best_epoch_accuracy_lr_{config.lr}_fps_{config.fps}_sample_{config.sample}_hidden_dim_{config.hidden_dim}_num_layers_{config.num_layers}_dropout_{config.dropout}.pth", name="best_model")
                    break
