...
def run_test(model, dataset, loader, criterion, device):
    ...
    if not os.path.exists(f"{configs.OUTPUT_DIR}/i3D-LSTM/test"):
        os.makedirs(f"{configs.OUTPUT_DIR}/i3D-LSTM/test")

    # Save the metrics to a text file
    with open(f"{configs.OUTPUT_DIR}/i3D-LSTM/test/metrics.txt", "w") as f:
...
if __name__ == "__main__":
    ...
    sweep_id = wandb.sweep(sweep_config, project="i3d+LSTM_test")    
    ...
    def sweep_test(config=None):
        with wandb.init(config=config) as run:
            ...
            annotation_file = f"{configs.DATASET_DIR}/clipped_videos/splits/test.txt"
            ...
            model_path =  f"{script_path}/checkpoints/lstm_best_epoch_accuracy_hidden_dim_{config.hidden_dim}_num_layers_{config.num_layers}_dropout_{config.dropout}_stream_{config.stream}_lr_{config.lr}_fps_{config.fps}_window_{config.window_and_step_size[0]}_step_{config.window_and_step_size[1]}.pth"
